...
class Anonimizer(ABC):
    def __init__(self, deep_anon:bool=False):
        ...
        self.annon_tags = Path(os.path.join(os.path.dirname(__file__),'Annon_DICOM_TAGs.m')).read_text()
    ...
    def anonimise_element(self, elem:DataElement)-> DataElement:
        ...
        # skipping VRs
        if elem.VR in ['AE', 'AS', 'AT', 'DS', 'FL', 'FD', 'IS', 'SL', 'SS', 'SV', 'UC', 'UL', 'US', 'UV']:
        ...
        # Anonimise all short and long text in dicom tags that need to be removed
            ...
            if elem.value != '' and str(elem.tag)[1:-1].replace(" ", '') in annon_tags:
                if 'NUMBER' in elem.name.upper() or 'SEQUENCE' in elem.name.upper() or 'ID' in elem.name.upper():
                    ...
                    elem.value = get_non_digits(encrypt_string(str(elem.value))) if elem.VR == 'LO' else get_digits(encrypt_string(elem.value))[:16]  # 'None'
        ...
        # annonimize sequences data
            ...
            raise NotImplementedError(f"Unknown VR {elem.VR}.Please report to the developers of dcm-anon.")
        return elem
